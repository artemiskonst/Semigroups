...
SemigroupHomomorphismByImages := function(S, T, gens, imgs)
  local map, R, rel, fun, gen, img, U;
  ...
  if not IsSemigroup(S) then
    ...
    ErrorNoReturn("the 3rd argument (a list) must consist of elements of the 1st argument (a semigroup)");
  elif Size(Semigroup(gens)) <> Size(S) then
    ErrorNoReturn("the 1st argument (a semigroup) is not generated by the 3rd argument (a list)");
  elif not ForAll(imgs, x-> x in S) then
    ErrorNoReturn("the 4th argument (a list) must consist of elements of the 2nd argument (a semigroup)");
  fi;

  if Size(gens)<> Size(imgs) then
    ErrorNoReturn("the 3rd argument (a list) and the 4th argument (a list) are not the same size");
  fi;
